<template>
	<div>
		...
		<quill-editor class="editor" :style="editorStyle" v-model="value" :ref="myQuillEditor" :options="editorOption"
...
<script>
	...
	function resolveToolbarOptions(toolbarPreset) {
		if (toolbarPreset === "comment") {
			return COMMENT_TOOLBAR_OPTIONS;
		}
	...
	var _EditorOption_ = function(page_this) {
		return {
			modules: {
				toolbar: {
					container: resolveToolbarOptions(page_this.toolbarPreset),
					handlers: {
						image(value) {
							if (value) {
								// 触发input框选择图片文件
			...
			placeholder: page_this.placeholder,
		};
	};
	...
	export default {
		...
		computed: {
			...
			editorStyle: function() {
				const heightValue = typeof this.editorHeight === "number" ? `${this.editorHeight}px` : String(this.editorHeight || "400px");
				return {
